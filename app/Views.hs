...
addGroceryForm = form_ [ hxpost_ "/grocery" ] (groceryInput <> button)
  where
    groceryInput = label_ (do toHtml ("Grocery" :: Text) 
                              input_ [class_ "add-grocery-input", name_ "name", type_ "text"])
    button = button_ "Add"

clearGottenForm :: Html ()
clearGottenForm = form_ [ class_ "clear-gotten", hxpost_ "/clear-gotten",  hxswap_ "none" ] $
    button_ "Clear"
...
renderGroceryRow (Grocery groceryId name active) = tr_ (checkCell <> labelCell)
  where
    ...
    formInputs = do
        input_ ([ class_ "active-chk", type_ "checkbox", name_ "active" ] <> [ checked_ | active ])
...
groceryList groceries = 
    table_ [class_ "grocery-list"] (tbody_ [ hxtarget_ "closest tr", hxswap_ "outerHTML" ] $ 
